...
import edu.yavirac.profesores.services.TeacherService;
...
public class TeacherController {
	...
	public ResponseEntity<List<Teacher>> getTeachers(@RequestParam(value = "name", required = false) String name) {
		...
		if (name == null) {
			teachers = _teacherService.finAllTeachers();
			if (teachers.isEmpty()) {
	...
	public static final String TEACHER_UPLOADED_FOLDER = "pictures/teacher/";

	// CREATE TEACHER IMAGE
	@RequestMapping(value = "/teachers/pictures", method = RequestMethod.POST, headers = ("content-type=multipart/form-data"))
	...
	// GET IMAGE
	@RequestMapping(value = "/teachers/{id_teacher}/pictures", method = RequestMethod.GET)
	...
	@RequestMapping(value = "/teachers/{id_teacher}/pictures", method = RequestMethod.DELETE, headers = "Accept=application/json")
	public ResponseEntity<?> deleteTeacherImage(@PathVariable("id_teacher") Long idTeacher) {
		...
		if (teacher.getAvatar().isEmpty() || teacher.getAvatar() == null) {
			return new ResponseEntity(new CustomErrorType("This Teacher dosen't have image assigned"),
		...
		if (teacher.getTeacherSocialMedias().size() == 0) {
			...
			while (i.hasNext()) {
				...
				if (teacherSocialMedia.getSocialMedia().getIdSocialMedia() == null
					...
					return new ResponseEntity(
							new CustomErrorType("We nee almost id_teacher, id_social_media and nickname"),
							HttpStatus.NO_CONTENT);
				} else {
					...
					if (tsmAux != null) {
						return new ResponseEntity(
								new CustomErrorType("The id social media "
										+ teacherSocialMedia.getSocialMedia().getIdSocialMedia() + " witch nickname: "
										+ teacherSocialMedia.getNickName() + " already exist"),
								HttpStatus.NO_CONTENT);
					}

					SocialMedia socialMedia = _socialMediaService
							.findById(teacherSocialMedia.getSocialMedia().getIdSocialMedia());
					if (socialMedia == null) {
						return new ResponseEntity(
								new CustomErrorType("The id social media "
										+ teacherSocialMedia.getSocialMedia().getIdSocialMedia() + " not found"),
								HttpStatus.NO_CONTENT);
					}
					...
					if (tsmAux == null) {
						...
						for (int j = 0; j < teacherSocialMedias.size(); j++) {
							...
							if (teacherSocialMedia.getTeacher().getIdTeacher() == teacherSocialMedia2.getTeacher()
									.getIdTeacher()
